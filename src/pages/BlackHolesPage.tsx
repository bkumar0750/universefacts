      ...
      <section className="space-y-6 animate-fade-in-up delay-200">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
          {blackHolesData.map((bh, idx) => (
            <div
              key={bh.id}
              className="glass-panel rounded-2xl border border-slate-200 dark:border-white/10 p-6 space-y-4 flex flex-col justify-between glass-panel-hover animate-fade-in-up"
              style={{ animationDelay: `${idx * 80}ms` }}
            >
              <div className="flex items-center justify-between">
                <span className="text-xs font-mono font-bold text-purple-700 dark:text-purple-300 bg-purple-500/10 px-2.5 py-1 rounded-full border border-purple-500/30">
                  {bh.type}
                </span>
                <span className="text-xs font-mono text-slate-600 dark:text-slate-400">{bh.distanceFromEarth}</span>
              </div>
