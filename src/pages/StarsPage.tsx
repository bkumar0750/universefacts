    ...
    <div className="space-y-12 pb-12">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
        {filteredStars.map((star) => (
          <div
            key={star.id}
            className="glass-panel rounded-2xl border border-slate-200 dark:border-white/10 p-6 space-y-4 flex flex-col justify-between"
          >
            <div className="flex items-center justify-between">
              <span className="text-xs font-mono text-amber-700 dark:text-amber-300 font-bold bg-amber-500/10 px-2.5 py-1 rounded-full border border-amber-500/30">
                {star.type}
              </span>
              <span className="text-xs font-mono text-slate-600 dark:text-slate-400">{star.distanceFromEarth}</span>
            </div>

            <div>
              <h3 className="text-2xl font-display font-bold text-slate-900 dark:text-white">{star.name}</h3>
              <span className="text-xs font-mono text-slate-600 dark:text-slate-400">Constellation: {star.constellation}</span>
