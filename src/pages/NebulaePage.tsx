  ...
  return (
    <div className="space-y-10 pb-12">
      ...
      <div className="space-y-3">
        <div className="inline-flex items-center gap-2 px-3 py-1 rounded-full text-xs font-mono font-semibold bg-indigo-500/10 border border-indigo-500/30 text-indigo-300">
        ...
        {types.map((type) => (
          <button
            ...
            className={`px-4 py-2 rounded-xl text-xs font-mono font-semibold transition-all ${
              filterType === type
                ? 'bg-indigo-500/20 text-indigo-300 border border-indigo-500/50 shadow-md font-bold'
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
        {filteredNebulae.map((nebula) => (
          <div
            key={nebula.id}
            className="glass-panel group rounded-2xl border border-white/10 overflow-hidden flex flex-col justify-between p-6 space-y-4"
          >
            <div className="flex items-center justify-between">
              ...
              <span className="text-xs font-mono text-slate-400">{nebula.distance}</span>
            </div>

            <div>
              <h3 className="text-2xl font-display font-bold text-white">{nebula.name}</h3>
              <span className="text-xs font-mono text-cyan-400">Constellation: {nebula.constellation}</span>
              <p className="text-xs text-slate-300 font-sans mt-2 leading-relaxed">
                {nebula.description}
              </p>
            </div>

            <div className="grid grid-cols-2 gap-2 text-xs font-mono pt-3 border-t border-white/10 text-slate-400">
              <div>Size: <strong className="text-white">{nebula.size}</strong></div>
            </div>
